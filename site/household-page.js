...
// Polyfill for Array.from (iOS 8-10 compatibility)
if (!Array.from) {
            ...
            contactsTable.innerHTML = `
                ...
                <tbody>
                    ${contacts.contacts.map(contact => {
                        ...
                        let contactedBy = getContactedBy(contact);
...
function displayHouseholdInfo(household) {
    ...
    // Build address link if address exists
    let addressHtml = '';
    if (household.address && household.address.street) {
...
function displayAssignedDeacons(assignments) {
    ...
    section.innerHTML = `<ul class="indented-list">${assignments.map(a => {
        if (a.deacon && a.deacon.firstName && a.deacon.lastName) {
            const displayName = `${a.deacon.firstName} ${a.deacon.lastName}`;
            const relevantTags = ['staff', 'elder', 'deacon', 'deaconess', 'helper'];
            const tagHtml = (a.deacon.tags || [])
                .filter(tag => relevantTags.includes(tag.toLowerCase()))
...
function displayMembers(members) {
    ...
    membersList.innerHTML = members.map(member => {
        // Build temporary location info if available
        let tempLocationHtml = '';
        if (member.temporaryAddress && member.temporaryAddress.isActive && member.temporaryAddress.locationId) {
            ...
            tempLocationHtml = `<div class="temp-location-info"></div>`;
        }
...
async function editMember(memberId) {
    try {
        ...
        // Show modal - both overlay and content
