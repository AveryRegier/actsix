...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const currentMemberId = (() => { try { return localStorage.getItem('memberId'); } catch (e) { return null; } })();
    const requestedDeaconMemberId = urlParams.get('deaconMemberId');
    const preferredAssignedMemberId = requestedDeaconMemberId || currentMemberId;
    ...
    if (isEditMode) {
        if (pageTitle) {
            pageTitle.innerHTML = '<span style="font-size:1.1em;">✏️</span>Edit Contact';
        }
        if (submitContactBtn) {
            submitContactBtn.textContent = 'Save Changes';
        }
        if (contactForm) {
            contactForm.action = '/form/edit-contact';
        }
        document.title = 'Edit Contact - Deacon Care System';
    }
    ...
    function applyContactToForm(contact) {
        ...
        const followUpInput = document.getElementById('follow-up-required');

        if (contactDateInput) {
            contactDateInput.value = formatDateForInput(contact.contactDate);
        }
        if (summaryInput) {
            summaryInput.value = contact.summary || '';
        }
        if (contactTypeSelect && contact.contactType) {
            contactTypeSelect.value = String(contact.contactType).toLowerCase();
        }
        if (followUpInput) {
            followUpInput.checked = Boolean(contact.followUpRequired);
        }

        const selectedMemberIds = new Set(Array.isArray(contact.memberId) ? contact.memberId : [contact.memberId]);
        const selectedDeaconIds = new Set(Array.isArray(contact.deaconId) ? contact.deaconId : [contact.deaconId]);

        Array.from(document.querySelectorAll('#membersSection input[name="memberId"]')).forEach((checkbox) => {
            checkbox.checked = selectedMemberIds.has(checkbox.value);
        });

        Array.from(document.querySelectorAll('#deaconsSection input[name="deaconId"]')).forEach((checkbox) => {
            checkbox.checked = selectedDeaconIds.has(checkbox.value);
        });
    }

    // Preserve where user came from so server can redirect back after submit.
    // Prefer explicit returnTo param, else use same-origin referrer that is not this page.
    if (returnToInput) {
        const explicitReturnTo = urlParams.get('returnTo');
        let resolvedReturnTo = '';
        if (explicitReturnTo) {
            resolvedReturnTo = explicitReturnTo;
        } else if (document.referrer) {
            try {
                const refUrl = new URL(document.referrer);
                if (refUrl.origin === window.location.origin && !refUrl.pathname.endsWith('/record-contact.html')) {
                    resolvedReturnTo = refUrl.pathname + refUrl.search + refUrl.hash;
                }
            } catch (e) {
                resolvedReturnTo = '';
            }
        }
        returnToInput.value = resolvedReturnTo;
    }
    ...
    if (contactIdInput && contactId) {
        contactIdInput.value = contactId;
    }

    // Dynamically preset contact date to today
    const contactDateInput = document.getElementById('contact-date');
    if (contactDateInput && !isEditMode) {
    ...
    try {
        ...
        for (var j = 0; j < assignedDeacons.length; j++) {
            ...
            var shouldPrecheck = preferredAssignedMemberId && assignment.deacon && assignment.deacon._id === preferredAssignedMemberId;
            deaconItems.push(
                '<div class="member-item">' +
                '<input type="checkbox" id="deacon-' + assignment.deacon._id + '" name="deaconId" value="' + assignment.deacon._id + '" ' + (shouldPrecheck ? 'checked' : '') + '>' +
            ...
            apiFetch('api/common-locations/' + tempMember.temporaryAddress.locationId)
                ...
                .then(data => {
                    ...
                    if (location && location.address) {
                        ...
                        const roomInfo = tempMember.temporaryAddress.roomNumber ? ' • Room/Unit: ' + tempMember.temporaryAddress.roomNumber : '';
        ...
        if (isEditMode && contactId) {
            const contactResponse = await fetch('api/contacts/' + encodeURIComponent(contactId));
            if (!contactResponse.ok) {
                throw new Error('Unable to load contact for editing');
            }
            const contactPayload = await contactResponse.json();
            applyContactToForm(contactPayload.contact);
        }
    } catch (error) {
...
document.getElementById('contact-form').addEventListener('submit', function(event) {
    ...
    var selectedAssignments = Array.from(document.querySelectorAll('#deaconsSection input:checked'));
    var contactTypeElement = document.getElementById('contact-type');
    var contactType = contactTypeElement ? String(contactTypeElement.value || '').toLowerCase() : '';
    ...
    // Require at least one checked assignment unless this is a note contact
    if (contactType !== 'note' && selectedAssignments.length === 0) {
        event.preventDefault();
        alert('Please select at least one person in Contacted by.');
        return false;
    }

    // Form submits to create or edit form route based on current mode.
    return true;
});
